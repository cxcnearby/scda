...
#include "smpmtpos.h"
  ...
  ifstream inselect;
  inselect.open("/home/changxc/mywork/code/repo/scda/data/"
                "bigevent_D100-201903200109-0135.txt");
  if (!inselect.is_open()) {
  ...
  t_match->Branch("time_b", &b_time_bs, "b_time_big/L");
  ...
  while (inselect >> b_tot >> b_entry >> b_x >> b_y >> b_anode_b >>
    ...
    for (Long64_t i = stamp[b_igcell]; i < smfee[b_igcell].size(); i++) {
      ...
      if (b_time_diff > -timewin && b_time_diff < -0) {
        ...
        stamp[b_igcell] = i;
        t_match->Fill();
      } else if (b_time_diff < 0 && b_time_diff > -0) {
        continue;
      } else if (b_time_diff < timewin && b_time_diff > 0) {
        b_fee_s = smfee[b_igcell][i];
        b_db = smdb[b_igcell][i];
        b_pmt = smpmt[b_igcell][i];
        b_anode_s = smanode[b_igcell][i];
