...
wcdaevent::wcdaevent(TTree *tree) : fChain(0) {
  ...
  if (tree == 0) {
    TFile *f = (TFile *)gROOT->GetListOfFiles()->FindObject(
        ...
        "ES.10001.WCDAEvent.20190215005726.007.root");
    if (!f || !f->IsOpen()) {
...
void wcdaevent::Init(TTree *tree) {
  ...
  fChain->SetBranchAddress("hits.anode_charge", hits_anode_charge,
                           &b_hits_anode_charge);
  fChain->SetBranchAddress("hits.dynode_charge", hits_dynode_charge,
                           &b_hits_dynode_charge);
  fChain->SetBranchAddress("hits.charge_over_range_id",
                           hits_charge_over_range_id,
                           &b_hits_charge_over_range_id);
  fChain->SetBranchAddress("hits.second", hits_second, &b_hits_second);
  fChain->SetBranchAddress("hits.year", hits_year, &b_hits_year);
  fChain->SetBranchAddress("hits.coarse_time", hits_coarse_time,
                           &b_hits_coarse_time);
  fChain->SetBranchAddress("hits.high_th_fine_time", hits_high_th_fine_time,
                           &b_hits_high_th_fine_time);
  fChain->SetBranchAddress("hits.low_th_fine_time", hits_low_th_fine_time,
                           &b_hits_low_th_fine_time);
  fChain->SetBranchAddress("hits.temprature_sensor_info",
                           hits_temprature_sensor_info,
                           &b_hits_temprature_sensor_info);
