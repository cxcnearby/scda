...
#include "smpmtpos.h"
  ...
  int fee_1; int db_1; int pmt_1; Long64_t time_1; Long64_t second_1; Long64_t
  ns_1; double anode_1; double dynode_1; int smevt_1; int evt_1; TTree
  *trpls=(TTree*)fsm->Get("trpls");
  ...
  */
  ...
  double sm_maxanode = -100;
  double sm_maxdynode = -100;
  double big_maxanode = -100;
  double big_maxdynode = -100;
  ...
  for (Long64_t jentry = bigevt0; jentry < bigevt1; jentry++) {
    ...
    if (jentry % 1000 == 0)
      cout << "bigevt=" << jentry << ",big_nhit=" << nhit << endl;
    big_nhit = nhit;
    big_evt = jentry;
    ...
    for (int ibig = 0; ibig < big_nhit; ibig++) {
      ...
      if (big_anode[ibig] > big_maxanode)
        big_maxanode = big_anode[ibig];
      if (big_dynode[ibig] > big_maxdynode)
        big_maxdynode = big_dynode[ibig];
    ...
    for (int i = smevt0; i < smsecond.size(); i++) {
      ...
      // printf("ibig=%d,
      // ism=%d,dftime=%f,smsecond=%ld,bigsecond=%ld,dfsecond=%ld\n",jentry, i,
      // dftime,smsecond,bigsecond,dfsecond);
      double timewin = 4000;
