// A version of event mode. this code use the
// wcdaevent data.
...
#include "smpmtpos.h"
  ...
  Float_t b_x, b_y;
  Long64_t b_entry_b, b_tot, b_time_b, b_evt, b_deltatime_b;
  double_t b_npe_b;
  // big pmt
  ...
  //     "tsm",
  //     "small event disorder: T_i - T_i-1"); // this tree is to store small
  //     pmt
  //                                           // time disorder data.
  ...
  for (Long64_t sjentry = 0; sjentry < snentries; sjentry++) {
    ...
    if (sm.anode_peak - sm.anode_ped > 0) {
      ...
      //   b_igcell_d =
      //   smpmtig_jd[smfee[smigcell].back()][smdb[smigcell].back()]
      //                          [smpmt[smigcell].back()];
      ...
      //   if (lastbutone < 0)
      //     lastbutone = 0;
      //   b_deltatime_d = smtime[smigcell].back() -
      //   smtime[smigcell][lastbutone]; t_smdisorder->Fill();
      // }
    }
  }
  ...
  for (Long64_t bjentry = 0; bjentry < bnentries; bjentry++) {
    ...
    b_entry_b = bjentry;
    b_evt = big.evt;
    for (int ihit = 0; ihit < big.nhit; ihit++) {
      if (!(big.hits_dynode_charge[ihit] > th_dynode))
        continue;
      b_tot++;
      b_fee_b = big.hits_fee[ihit];
      b_ch = big.hits_ch[ihit];
      b_igcell = bigpmtig[b_fee_b][b_ch];
      b_x = bigpmtx[b_fee_b][b_ch];
      b_y = bigpmty[b_fee_b][b_ch];
      b_anode_b = big.hits_anode_charge[ihit];
      b_dynode_b = big.hits_dynode_charge[ihit];
      Long64_t bigns = big.hits_low_th_fine_time[ihit] * 0.333 +
                       big.hits_coarse_time[ihit] * 16LL;
      b_time_b = bigns + big.hits_second[ihit] * 1000000000LL;
      ...
      for (Long64_t i = stamp[b_igcell]; i < smfee[b_igcell].size(); i++) {
        ...
        while (IfSearch && i < smfee[b_igcell].size() &&
               b_time_diff < timelow) {
          i0 = i;
          i += search_length;
  ...
  f_matchevents->Close();
  finish = clock();
  std::cout << "big events " << b_tot << " total time "
