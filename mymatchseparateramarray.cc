...
int smdb[900][20000];
int smpmt[900][20000];
int smanode[900][20000];
int smdynode[900][20000];
long smtime[900][20000];
long smtimedf[900][20000]; // small pmt
int where[900] = {0};
...
#include "smpmtpos.h"
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (anode_peak - anode_ped > 0) {
      smfee[smigcell][where[smigcell]] = fee;
      smdb[smigcell][where[smigcell]] = db;
      smpmt[smigcell][where[smigcell]] = pmt;
      smanode[smigcell][where[smigcell]] = anode_peak - anode_ped;
      smdynode[smigcell][where[smigcell]] = dynode_peak - dynode_ped;
      smtime[smigcell][where[smigcell]] =
          (second + 1) * 1000000000LL + ns * 20LL;
      where[smigcell]++;
    }
  }
  ...
  while (inselect >> b_tot >> b_entry >> b_evt >> b_igcell >> b_x >> b_y >>
    ...
    // if (b_entry == disentry)
    //   continue;
    for (Long64_t i = stamp[b_igcell]; i < 20000; i++) {
