...
void wcdaevent::Loop0() {
  ...
  bigevt1 = fChain->GetEntriesFast() - 1;
  LoadTree(bigevt1);
...
#include "smpmtpos.h"
  ...
  tr->Branch("iism", &iism, "iism[sm_nhit]/I");
  ...
  tr->Branch("sm_dftime", sm_dftime, "sm_dftime[sm_nhit]/D");
  ...
  tr->Branch("iibig", &iibig, "iibig[big_nhit]/I");
  ...
  tr->Branch("big_dftime", big_dftime, "big_dftime[big_nhit]/D");
  ...
  for (Long64_t jentry = bigevt0; jentry < bigevt1; jentry++) {
    ...
    for (int ibig = 0; ibig < big_nhit; ibig++) {
      ...
      big_dftime[ibig] = (big_second[ibig] - bigsecond) * 1000000000LL +
                         (big_ns[ibig] - bigns);
      if (big_dftime[ibig] < 1500) {
        tmean = tmean + big_dftime[ibig];
        nmean = nmean + 1;
      }
    }
    ...
    for (int i = smevt0; i < smsecond.size(); i++) {
      ...
      if (fabs(dftime) <= timewin) {
        ...
        if (jentry % 1000 == 0)
          cout << "bigevt=" << jentry << ",big_nhit=" << nhit
               << ",smnhit=" << smnhit << endl;
        smnhit++;
      }

      sm_nhit = smnhit;
      if (big_nhit >= 10000 || sm_nhit >= 10000)
        cout << "bignum,big_nhit=" << big_nhit << ",sm_nhit=" << sm_nhit
             << endl;

      for (int ibig = 0; ibig < big_nhit; ibig++) {
        big_dftime[ibig] = big_dftime[ibig] - tmean;
      }

      for (int ism = 0; ism < sm_nhit; ism++) {
        iism[ism] = ism;
        sm_dftime[ism] = sm_dftime[ism] - tmean;
      }

      for (int ibig = 0; ibig < big_nhit; ibig++) {
        iibig[ibig] = ibig;
        for (int j = 0; j < big_nhit - ibig - 1; j++) {
          if (big_dftime[j] > big_dftime[j + 1]) {
            swap(big_dftime[j], big_dftime[j + 1]);
            swap(big_anode[j], big_anode[j + 1]);
            swap(big_dynode[j], big_dynode[j + 1]);
            swap(big_second[j], big_second[j + 1]);
            swap(big_ns[j], big_ns[j + 1]);
            swap(big_pmtx[j], big_pmtx[j + 1]);
            swap(big_pmty[j], big_pmty[j + 1]);
            swap(big_pmtz[j], big_pmtz[j + 1]);
            swap(big_fee[j], big_fee[j + 1]);
            swap(big_ch[j], big_ch[j + 1]);
          }
        }
      }
