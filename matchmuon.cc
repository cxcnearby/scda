...
Long64_t bigsecond_evt0;;
Long64_t bigsecond_evtend;
int bigevt0=0;
int bigevt1=10000000;
...
printf("%ld,%ld",bigsecond_evt0,bigsecond_evtend);
cout<<"bigtime="<<bigsecond_evtend-bigsecond_evt0<<",entries="<<nentries<<endl;
  for (Long64_t jentry = 0; jentry < bigevt1; jentry++) {
...
b_dynode_b =dynode_charge;
  ...
  double bigns=low_th_fine_time*0.333+coarse_time*16;

  b_time_b = bigsecond*1000000000LL+bigns;
...
//  for (Long64_t jentry = 5530351; jentry < nentries; jentry++) {
 ...
 Long64_t ientry = LoadTree(jentry);
    ...
    nbytes += nb;
    if(fee>25) continue;
    if((second-bigsecond_evtend)>30) continue; 
  
cout<<"jsm="<<jentry<<",cha1=" <<second-bigsecond_evt0<<",cha2="<<second-bigsecond_evtend<<endl; 
    // if (Cut(ientry) < 0) continue;
    smigcell = smpmtig_jd[fee][db][pmt];
    if ((anode_peak - anode_ped > 0)&&(second-bigsecond_evt0)>-10) {
...
cout<<"nsm="<<smfee.size()<<endl;
  ...
  t_match->Branch("npe_s", &npe_s);
  t_match->Branch("npe_b", &npe_b);
...
// cout<<b_tot<<endl;
    ...
    //   continue;
      ...
      Double_t q1 = b_dynode_b/0.726/0.69;
      Double_t q = q0;
      if (q>160&&q1>0) q = q1;
      if (q<=0) q = 1.e-20;
    ...
    for (Long64_t i = stamp[b_igcell]; i < smfee[b_igcell].size(); i++) {
      ...
      //if (b_time_diff < timewin) {
      if (b_time_diff < 2500&&b_time_diff >500) {
        ...
        if(b_anode_s<4000) npe_s=b_anode_s*2;
        else npe_s=b_dynode_s*120*2.0;
