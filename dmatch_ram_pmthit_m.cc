...
#include "smpmtpos.h"
  ...
  Long64_t b_time_s_dynode; Long64_t b_time_s_anode; 
  ...
  for (Long64_t jentry = 0; jentry < nentries; jentry++) {
    ...
    if (anode_peak - anode_ped > 0) {
      ...
      smtime[smigcell].push_back((second + 1) * 1000000000LL + ns * 20LL - 64.34);
      smtime_anode[smigcell].push_back((second + 1) * 1000000000LL + ns * 20LL - 64.34 +anode_time);
      smtime_dynode[smigcell].push_back((second + 1) * 1000000000LL + ns * 20LL - 64.34 +dynode_time);
    }
  }
  ...
  t_match->Branch("anodetime_s", &b_time_s_anode);
  t_match->Branch("dynodetime_s", &b_time_s_dynode);
